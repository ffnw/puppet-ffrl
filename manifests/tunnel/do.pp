define ffrl::tunnel::do (
  ...
  if is_ip_address($transfer_net) {
    ...
    $pre_up    = [ "/sbin/iptables -t nat -A POSTROUTING ! -s ${_transfer_net} -o \$IFACE -j SNAT --to-source ${_nat_ip}" ] + $ip4_rule_up
    $post_up   = [ "/sbin/sysctl -w net.ipv4.conf.\$IFACE.rp_filter=0" ]
    $post_down = [ "/sbin/iptables -t nat -D POSTROUTING ! -s ${_transfer_net} -o \$IFACE -j SNAT --to-source ${_nat_ip}" ] + $ip4_rule_down
