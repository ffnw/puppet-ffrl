define ffrl::tunnel::do (
  ...
  if is_ip_address($transfer_net) {
    ...
    $f_transfer_net = ip_network($transfer_net)
    $f_nat_ip = ip_address($nat_ip)
    ...
    $pre_up    = [ '/sbin/iptables -t nat -A POSTROUTING ! -s ${f_transfer_net} -o $IFACE -j SNAT --to-source ${f_nat_ip}' ] + $ip4_rule_up
    $post_up   = [ '/sbin/sysctl -w net.ipv4.conf.$IFACE.rp_filter=0' ]
    $post_down = [ '/sbin/iptables -t nat -D POSTROUTING ! -s ${f_transfer_net} -o $IFACE -j SNAT --to-source ${f_nat_ip}' ] + $ip4_rule_down
